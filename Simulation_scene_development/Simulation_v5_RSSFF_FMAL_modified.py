...
class GripperPoseAngleController(Sofa.Core.Controller):
    ...
    def onAnimateEndEvent(self, event): # called at each end of animation step
        ...
        with self.root.Gripper.Gripper_Parts.GripperDOFs.position.writeableArray() as gripperPose:
        		...
        		#gripperPose[1] = [pose_stamped.pose.position.x, pose_stamped.pose.position.y, pose_stamped.pose.position.z, pose_stamped.pose.orientation.x, pose_stamped.pose.orientation.y, pose_stamped.pose.orientation.z, pose_stamped.pose.orientation.w]
        		#gripperPose[2] = [pose_stamped.pose.position.x, pose_stamped.pose.position.y, pose_stamped.pose.position.z, pose_stamped.pose.orientation.x, pose_stamped.pose.orientation.y, pose_stamped.pose.orientation.z, pose_stamped.pose.orientation.w]
        		
        with self.root.GoalPosition.Jaws_Goal_Angle.position.writeableArray() as goalAngle:
        		###print("\n\n"+str(angle_info.position[0])+"\n\n")
        		goalAngle[0,0]= (angle_info.position[0]*(-1)) + 0.3
        		goalAngle[1,0]= angle_info.position[0] - 0.3		
        """with self.root.Gripper.Articulations.position.writeableArray() as goalAngle:
        		###print("\n\n"+str(angle_info.position[0])+"\n\n")
        		goalAngle[0,0]= (angle_info.position[0]*(-1)) + 0.3
        		goalAngle[1,0]= angle_info.position[0] - 0.3"""
...
def createScene(root):
    ...
    GoalPosition = root.addChild('GoalPosition')
    GoalPosition.addObject('MechanicalObject', name="Jaws_Goal_Angle", template="Vec1d", position= [0, 0])
    #GoalPosition.addObject('MechanicalObject', name="Gripper_Goal_Pose", template="Rigid3d", position = [0, 0, 0, 0, 0, 0, 1])
    ...
    #Gripper_Parts.addObject('RestShapeSpringsForceField', name="RestShapeSFF", stiffness="10000", angularStiffness="10000", external_rest_shape="@../../GoalPosition/Gripper_Goal_Pose")
    
    Gripper.addObject('RestShapeSpringsForceField', name="RestShapeSFF", stiffness="10000", angularStiffness="10000", external_rest_shape="@../GoalPosition/Jaws_Goal_Angle")
