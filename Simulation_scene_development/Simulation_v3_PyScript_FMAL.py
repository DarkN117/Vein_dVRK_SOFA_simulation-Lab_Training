...
class GripperPoseAngleController(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, eventType): # called at each begin of animation step
        ...
        self.angleIncrement = 0.01 * (self.up - self.down)
        if self.modality == 0 :
        	print("\n\n   -ANGLE CONTROL-   \n\n")
        	with self.root.Gripper.Articulations.position.writeableArray() as jawAngle:
        		jawAngle[0] = jawAngle[0]-self.angleIncrement
        		jawAngle[1] = jawAngle[1]+self.angleIncrement	

        self.translationIncrement = 0.0005 * (self.up - self.down)
        if self.modality == 1 :
        	print("\n\n   -POSE CONTROL-   \n\n")
        	with self.root.Gripper.Gripper_Parts.GripperDOFs.position.writeableArray() as gripperPose:
        		gripperPose[0] = gripperPose[0] + [0, self.translationIncrement, 0, 0, 0, 0, 0]
        		gripperPose[1] = gripperPose[1] + [0, self.translationIncrement, 0, 0, 0, 0, 0]
        		gripperPose[2] = gripperPose[2] + [0, self.translationIncrement, 0, 0, 0, 0, 0]
        		
        """with self.root.Gripper.Articulations.position.writeableArray() as jawAngle:
        	jawAngle[0] = jawAngle[0]-0.05
        	jawAngle[1] = jawAngle[1]+0.05
        
        with self.root.Gripper.Gripper_Parts.GripperDOFs.position.writeableArray() as gripperPose:
        	gripperPose[0] = gripperPose[0] + [0.0002, 0.0001, 0, 0, 0, 0, 0]
        	gripperPose[1] = gripperPose[1] + [0.0002, 0.0001, 0, 0, 0, 0, 0]
        	gripperPose[2] = gripperPose[2] + [0.0002, 0.0001, 0, 0, 0, 0, 0]"""
        

    def onAnimateEndEvent(self, event): # called at each end of animation step
        if self.modality == 1:
        	with self.root.Gripper.Articulations.position.writeableArray() as jawAngle:
        			jawAngle[0] = self.restJawAngle
        			jawAngle[1] = -self.restJawAngle
        time = self.root.time.value
        if self.iteration==0: #solve a bug about time display at initializiation
        	time = 0
        print("\n:::::::::: Time elapsed: " + str(time) + " [s], iteration #" + str(self.iteration)+ " :::::::::: "+ str(self.restJawAngle))
        self.iteration += 1
...
def createScene(root):
    ...
    root.addObject('LocalMinDistance', name="LMD-proximity" , alarmDistance=0.001, contactDistance=0.0005, useLMDFilters=0)
    ...
    Gripper.addObject('MechanicalObject', name="Articulations", template="Vec1d", position=[-0.2, 0.2])
    ...
    #shuld be used to detect when two objects enter in contact, for example to stop jaw rotation...still to be understood better...
    listener = root.addObject(
        "ContactListener",
        name="TestContactListener",
        collisionModel1=Vein_collision.PointCollisionModel.getLinkPath(),
        collisionModel2=Gripper.Gripper_Parts.Right_Jaw_Collision.TriangleCollisionModel.getLinkPath() )
...
def main():
    ...
    #root.Gripper.Articulations.position.value
    
    Sofa.Simulation.animate(root, root.dt.value)
    
    #Change value of gripper angle at runtime:
    #while not root.dt.value == 2:
    #	root.Gripper.Articulations.position.value = [ (-0.175-(root.time.value/10)), (0.175+(root.time.value/10)) ]
    #	gripperAngle = root.Gripper.Articulations.position.value
    #	print(gripperAngle)
    #	print("\n")
    #	Sofa.Simulation.animate(root, root.dt.value)
