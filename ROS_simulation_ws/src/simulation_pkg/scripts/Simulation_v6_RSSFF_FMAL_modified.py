...
def position_plotter():
    ...
    #print(t_values)
    #print(pos_values)
    np.savetxt("time_data_FMAL.csv", 
           t_values,
           delimiter =", ",  # Set the delimiter as a comma followed by a space
           fmt ='% f')  # Set the format of the data as float
    np.savetxt("position_data_FMAL.csv", 
           pos_values,
           delimiter =", ",  # Set the delimiter as a comma followed by a space
           fmt ='% f')  # Set the format of the data as float
    ...
    plt.xlabel('Time')
    plt.ylabel('Position')
    plt.title('Vein node 111 - y coordinate vs time')
    plt.show()
...
def main():
	...
	#rospy.Subscriber("PSM_Pose_info", geometry_msgs.msg.PoseStamped, pose_stamped_callback)
	rospy.Subscriber("dvrk/PSM2/position_cartesian_current", geometry_msgs.msg.PoseStamped, pose_stamped_callback)
	rospy.Subscriber("PSM_Angle_info", sensor_msgs.msg.JointState, jaws_angle_callback)
...
class GripperPoseAngleController(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, eventType): # called at each begin of animation step
        ...
        if self.spawn_grid == 1:
        	...
        	self.root.Vein.Vein_visual.addObject('VisualGrid', plane="z", size=1, nbSubdiv=1000, thickness=0.1, color=[0.8, 1, 0.8])#color=[0.5, 0.9, 0.1]
        	self.spawn_grid = 0
    ...
    def onAnimateEndEvent(self, event): # called at each end of animation step
        ...
        if plot==1:
        	...
        	pos_values.append(self.root.Vein.dofs.position.value[111][1])
        
        #print("\n:::::::::: Data from dVRK PSM ::::::::::\n____\nPOSE:\n"+str(pose_stamped)+"\n____\nJAWS ANGLE:\n"+str(angle_info)+"\n____\n\n")
        
        with self.root.Shaft_target_pose.rest_shape.position.writeableArray() as gripperGoalPose:
        		gripperGoalPose[0] = [pose_stamped.pose.position.x, -pose_stamped.pose.position.z, pose_stamped.pose.position.y, pose_stamped.pose.orientation.x, pose_stamped.pose.orientation.y, pose_stamped.pose.orientation.z, pose_stamped.pose.orientation.w]
        		print(gripperGoalPose)
        with self.root.Jaws_target_angle.rest_shape.position.writeableArray() as gripperGoalAngle:
        		###print("\n\n"+str(angle_info.position[0])+"\n\n")
        		gripperGoalAngle[0,0]= (angle_info.position[0]*(-1)) + 0.3
        		gripperGoalAngle[1,0]= angle_info.position[0] - 0.3
        ...
        print( "\n++++++++++++++++++++\nVein mesh point index 111 position:" + "\n x: " + str(self.root.Vein.dofs.position.value[111][0])+ " [m]\n y: " + str(self.root.Vein.dofs.position.value[111][1])+ " [m]\n z: " + str(self.root.Vein.dofs.position.value[111][2])+ " [m]" )
...
def createScene(root):
    ...
    #root.addObject('BackgroundSetting', image="white_background.jpg")
    root.addObject('BackgroundSetting', color="0.35 0.05 0.15") 
    ...
    root.addObject('LocalMinDistance', name="LMD-proximity" , alarmDistance=0.0015, contactDistance=0.00075, useLMDFilters=0)
    ...
    # VEIN --- Lenght=120mm --- diameter=13mm +++++ SOFA DIMENSIONS ARE TAKEN FROM MODELS/MESH DEFINITION: I defined models in meters so,for example, 13mm become 0.013 here
    ...
    #CAN DECIDE BETWEEN DIFFERENT TYPES OF INTERNAL FORCES MODELS
    ...
    #Vein.addObject('FastTetrahedralCorotationalForceField',template="Vec3d", youngModulus=100000, poissonRatio=0.4, method="large")
    #Vein.addObject('TetrahedronFEMForceField',template="Vec3d", youngModulus=100000, poissonRatio=0.4, method="large", computeGlobalMatrix="0", updateStiffnessMatrix="false")
    #Vein.addObject('TetrahedronHyperelasticityFEMForceField', name="FEM", topology="@topo", ParameterSet="3448.2759 31034.483")
    #'materialName' parameter can be : "ArrudaBoyce", "StVenantKirchhoff", "NeoHookean", "MooneyRivlin", "VerondaWestman", "Costa", "Ogden" or none
    #'ParameterSet' dependes on the model we choose
    ...
    #Vein.addObject('LinearSolverConstraintCorrection', name="Vein_ConstraintCorrection")
    ...
    Base.addObject('MechanicalObject', name="Base_point", template="Rigid3d")
    #Base.addObject('LinearSolverConstraintCorrection', name="Base_ConstraintCorrection")
    Base.addObject('GenericConstraintCorrection', name="Base_ConstraintCorrection")
    ...
    Shaft_target_pose.addObject('MechanicalObject', name="rest_shape", template="Rigid3d", position=[0, 0.03, 0, 0, 0, 0, 1])
    
    Shaft_pose = root.addChild('Shaft_pose')
    Shaft_pose.addObject('EulerImplicitSolver')
    Shaft_pose.addObject('SparseLDLSolver', template="CompressedRowSparseMatrix")
    Shaft_pose.addObject('MechanicalObject', name="shaft_pose", template="Rigid3d", position=[0, 0.03, 0, 0, 0, 0, 1])
    #Shaft_pose.addObject('LinearSolverConstraintCorrection', name="Shaft_ConstraintCorrection")
    Shaft_pose.addObject('GenericConstraintCorrection', name="Shaft_ConstraintCorrection")
    Shaft_pose.addObject('RestShapeSpringsForceField', name="RestShapeSFF", stiffness="10000", angularStiffness="10000", external_rest_shape="@../Shaft_target_pose/rest_shape")  
    
    Jaws_target_angle = root.addChild('Jaws_target_angle')
    Jaws_target_angle.addObject('MechanicalObject', name="rest_shape", template="Vec1d", position=[0, 0])
    ...
    Gripper.addObject('EulerImplicitSolver')
    Gripper.addObject('SparseLDLSolver', template="CompressedRowSparseMatrix") #if I use SVDLinearSolver program do not goes to segmentation fault after I press animate but jaws rotation is unstable
    Gripper.addObject('MechanicalObject', name="Articulations", template="Vec1d", position=[0, 0])
    Gripper.addObject('RestShapeSpringsForceField', name="RestShapeSFF", stiffness="10000", angularStiffness="10000", external_rest_shape="@../Jaws_target_angle/rest_shape")
    #Gripper.addObject('LinearSolverConstraintCorrection', name="Gripper_ConstraintCorrection")
    Gripper.addObject('GenericConstraintCorrection', name="Gripper_ConstraintCorrection")
    ...
    Gripper_Parts.addObject('ArticulatedSystemMapping', input1="@../Articulations", input2="@../../Shaft_pose/shaft_pose", output="@GripperDOFs")
