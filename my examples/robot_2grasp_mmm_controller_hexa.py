...
def createScene(rootNode):
	...
	rootNode.addObject('RequiredPlugin', name='Sofa.Component.Mass') # Needed to use components [MeshMatrixMass]  
	...
	rootNode.addObject('VisualStyle', displayFlags='showCollisionModels showBehaviorModels hideMappings showForceFields')
	rootNode.addObject('DefaultAnimationLoop')
	rootNode.addObject('DefaultVisualManagerLoop')

	meshDivision = rootNode.addChild('meshDivision')
	meshDivision.addObject('RegularGridTopology', name='topology', n=[12, 12, 3] , min=[-0.5, 0.0, -0.05], max=[0.5, 1.0, 0.05])
	...
	boxWhole= meshDivision.addObject('BoxROI',template="Vec3d", name='box_roi_whole', box=[-0.51, -0.01, -0.11, 0.51, 1.01, 0.11], position="@beamMesh.position", drawBoxes=True )
	boxRigid= meshDivision.addObject('BoxROI',template="Vec3d", name='box_roi_rigid', box=[-0.1, 1.01, -0.1, 0.1, 0.9, 0.1], position="@beamMesh.position", drawBoxes=True)
	boxRigid2= meshDivision.addObject('BoxROI',template="Vec3d", name='box_roi_rigid2', box=[-0.1, -0.01, -0.1, 0.1, 0.1, 0.1], position="@beamPart1Mech.position", drawBoxes=True)
	...
	SolverNode.addObject("EulerImplicitSolver", name="odeSolver", rayleighStiffness="0.1",  rayleighMass="0.1", vdamping="1.0")
	...
	defObjPos = getComplementaryPointsInROI(rootNode.meshDivision.box_roi_whole, rootNode.meshDivision.box_roi_rigid, rootNode.meshDivision.box_roi_rigid2)
	defObjInd = getComplementaryIndices(rootNode.meshDivision.box_roi_whole, rootNode.meshDivision.box_roi_rigid, rootNode.meshDivision.box_roi_rigid2)
	...
	RigidNode.addObject("MechanicalObject",template="Rigid3d",name="rigid1", position=[0, 1, 0,  0, 0, 0, 1], showObject=True, showObjectScale=0.1)
	...
	RigidNode2.addObject("MechanicalObject",template="Rigid3d",name="rigid2", position=[0, 0, 0,  0, 0, 0, 1], showObject=True, showObjectScale=0.1)
	RigidNode2.addObject('FixedConstraint', indices='0')

	RigidifiedNode2= RigidNode2.addChild('RigidifiedNode2')
	RigidifiedNode2.addObject("MechanicalObject",name="rigidMecha",template="Vec3d", position="@"+boxRigid2.getPathName()+".pointsInROI")
	...
	FEMNode.addObject('HexahedronFEMForceField', name='HexaFF', src="@meshInput", poissonRatio=0.3, youngModulus=10000, method="polar")
	...
	RigidNode.addObject( ControlParticles(name="MouvementPinces", Particle1=RigidNode.rigid1, Particle2=RigidNode2.rigid2, root=rootNode) )
