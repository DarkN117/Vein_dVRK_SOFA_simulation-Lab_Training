...
class RotationController(Sofa.Core.Controller):
    ...
    def rotate(self):
        ...
        if self.iterations > self.max_iterations:
            self.iterations = 0
            self.other_direction = not self.other_direction #change rotation direction once we reached 360°
...
def createScene(root):
    root.addObject('RequiredPlugin', name='Sofa.Component.Engine.Transform') # Needed to use components [TransformEngine]  
    root.addObject('RequiredPlugin', name='Sofa.Component.IO.Mesh') # Needed to use components [MeshOBJLoader]  
    root.addObject('RequiredPlugin', name='Sofa.Component.Mapping.Linear') # Needed to use components [IdentityMapping]  
